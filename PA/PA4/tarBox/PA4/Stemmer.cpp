...
Stemmer::Stemmer(){
	...
	//regionTwoC1 = regex("(
	regionTwoC1 = regex("(.*?[aeiou][^aeiou]{1})(.*)");
	regionTwoC2 = regex("(.*?[^aeiou]y[^aeiou]{1})(.*)");
	regionTwoC3 = regex("(y[^aeiou]{1})(.*)");
...
string Stemmer::region2(const string& word){
	
	smatch region1Match;
	//regex regionOne = regex("((.*?[aeiou]|.*?[^aeiou]y|^yy)[^aeiou]{1})(.*)");
	//regex_match(word, region1Match, regionOne);
	//regex regionTwoC1 = regex("(.*?[aeiou][^aeiou]{1})(.*)");
	//regex regionTwoC2 = regex("(.*?[^aeiou]y[^aeiou]{1})(.*)");
	//regex regionTwoC3 = regex("(y[^aeiou]{1})(.*)");
	unsigned int shortest = 0;
	string regionTwo = string("");
	string regionOne = region1(word);
	if(regex_match(regionOne, region1Match, regionTwoC1)){
		shortest = string(region1Match[1]).length();
		regionTwo = region1Match[2];
	}
	if(regex_match(regionOne, region1Match, regionTwoC2)){
		if(string(region1Match[1]).length()<shortest || shortest==0){
			shortest = string(region1Match[1]).length();
			regionTwo= region1Match[2];
		}
	}
	if(regex_match(regionOne, region1Match, regionTwoC3)){
		if(string(region1Match[1]).length()<shortest|| shortest==0){
			shortest = string(region1Match[1]).length();
			regionTwo= region1Match[2];
		}
...
bool Stemmer::rule7(string& word){
	...
	string tempRegion = region2(word);
