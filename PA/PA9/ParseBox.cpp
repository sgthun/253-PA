...
ParseBox::ParseBox(){
	...
	containsAlphaNumeric = regex(".*[a-zA-Z0-9']+.*");
}
bool ParseBox::hasAlphaNum(const string& word){
	return regex_match(word, containsAlphaNumeric);
}
bool ParseBox::isAlphaNum(const string& word){
	return regex_match(word, alphaNumeric);
}
bool ParseBox::purePuncCheck(const string& word){
	return regex_match(word, purePunc);
}
