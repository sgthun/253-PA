...
public:
	...
	WordCounts(const Replace& exec):exceptionTable(exec), letterCounter(0), wordCounter(0), sentenceCounter(0){}
	
	
	int scanFileIn(istream& inFile, string file);
	void sortList();
	void printList();
	void countWords();
	int stringLength();
	void updateLevelInfo(const string& word);
	double L();
	double S();
	double CLindex();
	string getFileName();
	//~WordCounts();
	void implicitSentence();
